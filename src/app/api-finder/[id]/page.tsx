                    ...
                    {Array.isArray(recipe.extendedIngredients) && recipe.extendedIngredients.length > 0 ? (
                      recipe.extendedIngredients.map(
                        (ing: SpoonacularExtendedIngredient, index: number) => (
                          <li key={`${ing.id ?? ing.originalName}-${index}`} className="text-gray-800">{ing.original}</li>
                        )
                      )
                    ) : (
                  ...
                  {steps && steps.length > 0 ? (
                    <ol className="list-decimal list-inside space-y-4 bg-gray-50 p-4 rounded shadow-sm">
                      {steps.map((step: SpoonacularInstructionStep) => (
                        <li key={step.number} className="mb-2 text-gray-800 marker:font-semibold">
                          {step.step}
                        </li>
                      ))}
                    </ol>
                  ) : plainInstructions ? (
                       <div className="prose prose-sm max-w-none bg-gray-50 p-4 rounded shadow-sm" dangerouslySetInnerHTML={{ __html: plainInstructions }} />
                  ) : (
                    <p className="text-gray-500 italic bg-gray-50 p-4 rounded shadow-sm">No detailed instructions provided.</p>
                  )}
                </div>
              </div>
   );
 ...
 {/*
   return (
      <div className="p-4 md:p-8 max-w-3xl mx-auto">
        <h1 className="text-xl font-bold mb-4">Basic Recipe Data Received:</h1>
        <p>
          <strong className="font-semibold">ID:</strong> {recipe.id}
        </p>
        <p>
          <strong className="font-semibold">Title:</strong> {recipe.title}
        </p>
